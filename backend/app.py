...
@app.get("/players")
def get_players():
    return [{"personId":playerid,"playerName":name} for playerid,name in sorted(id_to_name.items(),key=lambda x:x[1]) 
            if name and str(name).strip()]

@app.get("/leaderboard")
def get_leaderboard(limit: int=Query(default=10,ge=1,le=100),min_attempts:int=Query(default=15,ge=0,le=60)):
    ...
    cols=['personId','playerNameI','fire_score','clutch_points','clutch_attempts','clutch_fg_pct','clutch_hot_rate','clutch_avg_streak','clutch_max_streak','overall_hot_rate','overall_avg_streak','overall_max_streak']
    return {
